<?php
// Email Forwarder Service - Handles email sending separately from data processing
...
$required_fields = ['name', 'email', 'organization', 'project', 'date', 'time', 'booking_id'];
$missing_fields = [];
...
if (!empty($missing_fields)) {
    http_response_code(400);
    echo json_encode(['error' => 'Missing required fields for email: ' . implode(', ', $missing_fields)]);
    exit();
}
...
if ($email_sent) {
    ...
    echo json_encode([
        'success' => false,
        'error' => 'Email failed to send: ' . $email_error,
