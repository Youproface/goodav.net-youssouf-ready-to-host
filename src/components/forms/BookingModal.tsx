        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center 
          ...
          <div className="bg-[#1b1b1d] w-[90%] max-w-3xl rounded-xl shadow-lg text-white p-6 md:p-8  mx-auto">
            ...
            <div className="mt-6">
              ...
              {step === 7 && <Step7 meetingSoftware={meetingSoftware} setMeetingSoftware={setMeetingSoftware} handleFormSubmit={handleFormSubmit} submitting={submitting} />}
            </div>
...
------------------------- */
function Step7({ meetingSoftware, setMeetingSoftware, handleFormSubmit, submitting }) {
        ...
        <Tooltip.Provider>
          <Tooltip.Root>
            <Tooltip.Trigger asChild>
              <button
                onClick={handleFormSubmit}
                className={`flex items-center px-8 py-3 rounded-lg text-white font-semibold ${
                  submitting || !meetingSoftware
                    ? 'bg-gray-500 cursor-not-allowed'
                    : 'bg-orange-600 hover:bg-orange-700'
                }`}
              ...
              >
                <Calendar className="w-5 h-5 mr-2" />
                <span>{submitting ? 'Scheduling...' : 'Schedule Consultation'}</span>
              </button>
            </Tooltip.Trigger>
            {(!meetingSoftware || submitting) && (
              <Tooltip.Portal>
                <Tooltip.Content
                  className="bg-gray-900 text-white px-4 py-3 rounded-lg text-sm shadow-2xl border-2 border-gray-600 max-w-xs z-[10001]"
                  sideOffset={8}
