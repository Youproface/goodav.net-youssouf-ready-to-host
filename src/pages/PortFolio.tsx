...
const ALL_VIDEOS = [...HOME_VIDEOS, ...PORTFOLIO_VIDEOS, ...VERTICAL_VIDEOS];
...
export default function PortFolio() {
	...
	function autoLabel(video, source) {
		...
		if (
			/event|summit|conference|ceremony|festival|gala|performance|show|talk show|launch|opening|closing|day|week|session|recap|highlight|feature|behind-the-scenes/.test(
				t
			)
		)
			return 'Event';
	...
	function smartLabel(video, source) {
		...
		if (source === 'Featured') return 'Featured';
		if (
			/documentary|story|chapter|transform|review|journey|change|empowering|market|building|peace|prosperity|sitcom|series|film|promo|holiday|greetings|gift|ad|teaser|trailer|flashback|inspiring|impact|achievements|initiative|empowering|empower|celebrating|challenges|talk show/.test(
				t
			)
		)
			return 'Documentary';
		if (
			/event|summit|conference|ceremony|festival|gala|performance|show|launch|opening|closing|day|week|session|recap|highlight|feature|behind-the-scenes|gathering|reel|celebrating|class|graduation|mo|mou|session|flag-off|launch|promo|gala|gathering|celebration|award|presentation|workshop|training|meeting|panel|forum|webinar|recap|review|report|update|briefing|kickoff|rollout|release|unveiling|commemoration|anniversary|milestone|completion|flag-off|flag off|flagoff/.test(
				t
			)
		)
			return 'Event';
		if (
			/corporate|client|company|brand|business|commercial|promo|ceo|academy|school|college|institute|organization|ngo|agency|authority|project|programme|initiative|summit|conference|partnership|agreement|mou|signing|launch|rollout|release|unveiling|commemoration|anniversary|milestone|completion|flag-off|flag off|flagoff/.test(
				t
			)
		)
			return 'Corporate';
	...
	const filtered = useMemo(() => {
		return ENRICHED_VIDEOS.filter((v) => {
			...
			return false;
		}).filter((v) => (query.trim().length === 0 || v.title.toLowerCase().includes(query.toLowerCase())));
	...
	return (
		<>
			...
			<main className="bg-[#0f1012] text-zinc-100 min-h-screen px-4 py-0 relative">
				...
				<header
					className="relative mt-10 py-32 px-4 -mx-4 sm:-mx-6 md:-mx-8 bg-transparent text-center mb-10 flex flex-col items-center justify-center min-h-[340px]"
					role="banner"
				>
					...
					<div className="relative max-w-6xl mx-auto">
						...
						>
							...
							<strong>Storytelling, innovation, and impact</strong> are at the heart of every project.
						</p>
					</div>
				</header>
				...
				<div className="max-w-7xl mx-auto relative">
						...
						<div className="max-w-7xl mx-auto px-1 sm:px-2 md:px-0 py-3 flex flex-col gap-3">
							<div className="flex flex-wrap items-center justify-between gap-3">
										...
										return (
											<button
												...
												className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
													category === tab ? 'bg-primary text-primary-foreground shadow-glow' : 'bg-white/5 hover:bg-white/10'
												}`}
								...
								<div className="flex items-center gap-2 w-full sm:w-auto">
									...
									/>
									<label className="sr-only" htmlFor="portfolio-sort">
										Sort
									</label>
									<select
										id="portfolio-sort"
							...
							<div className="text-xs text-zinc-400">
								Showing{' '}
								<span className="text-zinc-200 font-medium">{Math.min(page * PAGE_SIZE, sorted.length)}</span> of{' '}
								<span className="text-zinc-200 font-medium">{sorted.length}</span> videos
							</div>
						</div>
					</section>
					...
					{selectedVideo && (
						...
						>
								...
								{!isPlaying ? (
									<>
										...
										<img
											...
											alt={
												ALL_VIDEOS.find((v) => v.id === selectedVideo)?.title + ' - Video thumbnail'
											}
									...
									<>
										<iframe
											...
											title={
												ALL_VIDEOS.find((v) => v.id === selectedVideo)?.title + ' - YouTube video player'
											}
											frameBorder="0"
							...
							<h1 className="sr-only">{ALL_VIDEOS.find((v) => v.id === selectedVideo)?.title}</h1>
						</article>
					)}
							...
							return (
								...
								>
									<button
										...
										tabIndex={0}
										onKeyDown={(e) => {
											if (e.key === 'Enter' || e.key === ' ') handleSelectVideo(video.id);
										}}
									>
										...
										<div className={`absolute left-0 right-0 bottom-0 px-4 pb-3 pt-2 text-white fade-up delay-450`}>
											...
											<p
												className="text-xs text-zinc-300 mt-1 text-left"
												aria-label="Client"
											>
												{video.client}
											</p>
											<div className="flex items-center justify-between mt-2">
												<div
													className="flex items-center gap-1 text-xs text-zinc-200"
													aria-label="Year"
												>
													<svg
														className="w-4 h-4 inline-block mr-1"
